...
/*
Additions have been made to the library which includes several functions
from the dynamixel controltable.
This was done by Jón Davíð Ragnarsson and Oliver Emil Trudslev Hansen.
jragna16@student.aau.dk & oeth16@student.aau.dk
*/
...
//Added by group 365
void DynamixelClass::setLimitPWM(unsigned char ID, unsigned int pwm)
{
 pwm %= 885;
 ...
 writeN(ID, 0x24, arr, 2);
}
//Added by group 365
void DynamixelClass::setLimitCurrent(unsigned char ID, unsigned int cur)
{
 cur %= 1941;
 ...
 writeN(ID, 0x26, arr, 2);
}

//Added by group 365
void DynamixelClass::setGoalCurrent(unsigned char ID, int curr){
...
void DynamixelClass::setGoalPWM(unsigned char ID, unsigned int pwm){
 ...
 writeN(ID, 0x64, arr, 2);
}

//Added by group 365
void DynamixelClass::setPGain(unsigned char ID, unsigned int p_gain){
 unsigned char arr[] = {
   (p_gain & 0xFF),
   (p_gain & 0xFF00) >> 8
 };
 writeN(ID, 0x54, arr, 2);
}

//Added by group 365
void DynamixelClass::setIGain(unsigned char ID, unsigned int i_gain){
 unsigned char arr[] = {
   (i_gain & 0xFF),
   (i_gain & 0xFF00) >> 8
 };
 writeN(ID, 0x52, arr, 2);
}

//Added by group 365
void DynamixelClass::setDGain(unsigned char ID, unsigned int d_gain){
 unsigned char arr[] = {
   (d_gain & 0xFF),
   (d_gain & 0xFF00) >> 8
 };
 writeN(ID, 0x50, arr, 2);
}

//Added by group 365
int DynamixelClass::getGoalCurrent(unsigned char ID)
{
 clearRXbuffer();
 readN(ID, 0x66, 2);                   //Read from adress 0x7E (Present Load), byte size 4 (should be 2?)
...
//Added by group 365
int  DynamixelClass::getPWM(unsigned char ID)
{

 clearRXbuffer();
 readN(ID, 0x7c, 2);                   //Read from adress 0x7E (Present Load), byte size 4 (should be 2?)
 getParameters();                      //Filters parameters from ReturnPacket
...
//Added by group 365
int  DynamixelClass::getVoltage(unsigned char ID)
{

 clearRXbuffer();
 readN(ID, 0x90, 2);                   //Read from adress 0x7E (Present Load), byte size 4 (should be 2?)
 getParameters();                      //Filters parameters from ReturnPacket
...
//Added by group 365
void DynamixelClass::setMode(unsigned char ID,int mode)
{
 unsigned char arr[1] = {mode};
...
int DynamixelClass::getPosition(unsigned char ID){
 ...
 int sum;
 sum = (data[2] << 8) | data[1];       //Converting two information bytes into a integer (position dat
 return sum;
}
...
int DynamixelClass::writeSN(unsigned char ID, unsigned short addr, char *arr, int n){
